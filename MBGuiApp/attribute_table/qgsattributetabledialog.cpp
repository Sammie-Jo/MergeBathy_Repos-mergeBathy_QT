    ...
    , mCurrentSearchWidgetWrapper( nullptr )
{
  ...
  setStyleSheet( MBGuiApp::instance()->styleSheet() );
  ...
  context.setVectorLayerTools( MBGuiApp::instance()->vectorLayerTools() );
  ...
  {
    QgsMapCanvas *mc = MBGuiApp::instance()->mapCanvas();
  ...
  mMainView->init( mLayer, MBGuiApp::instance()->mapCanvas(), r, context );
  ...
  connect( this, SIGNAL( saveEdits( QgsMapLayer * ) ), MBGuiApp::instance(), SLOT( saveEdits( QgsMapLayer * ) ) );

  bool myDockFlag = settings.value( "/qgis/dockAttributeTable", false ).toBool();
  if ( myDockFlag )
  {
    mDock = new QgsAttributeTableDock( tr( "%1 (%n Feature(s))", "feature count", mMainView->featureCount() ).arg( mLayer->name() ), MBGuiApp::instance() );
    mDock->setWidget( this );
    connect( this, SIGNAL( destroyed() ), mDock, SLOT( close() ) );
    MBGuiApp::instance()->addDockWidget( Qt::BottomDockWidgetArea, mDock );
  }
...
void QgsAttributeTableDialog::keyPressEvent( QKeyEvent* event )
{
  ...
  {
    MBGuiApp::instance()->deleteSelected( mLayer, this );
  }
}
...
{
  //MBGuiApp::instance()->saveEdits( mLayer, true, true );
}
...
void QgsAttributeTableDialog::on_mToggleEditingButton_toggled()
{
  if ( !mLayer )
    return;
//  if ( !MBGuiApp::instance()->toggleEditing( mLayer ) )
//  {
...
void QgsAttributeTableDialog::on_mRemoveAttribute_clicked()
{
  ...
  if ( dialog.exec() == QDialog::Accepted )
  {
    ...
    {
      MBGuiApp::instance()->messageBar()->pushMessage( tr( "Attribute error" ), tr( "The attribute(s) could not be deleted" ), QgsMessageBar::WARNING, MBGuiApp::instance()->messageTimeout() );
      mLayer->destroyEditCommand();
    }
    // update model - a field has been added or updated
    masterModel->reload( masterModel->index( 0, 0 ), masterModel->index( masterModel->rowCount() - 1, masterModel->columnCount() - 1 ) );
...
void QgsAttributeTableDialog::setFilterExpression( const QString& filterString )
{
  ...
  {
    MBGuiApp::instance()->messageBar()->pushMessage( tr( "Parsing error" ), filterExpression.parserErrorString(), QgsMessageBar::WARNING, MBGuiApp::instance()->messageTimeout() );
    return;
  }
  ...
  {
    MBGuiApp::instance()->messageBar()->pushMessage( tr( "Evaluation error" ), filterExpression.evalErrorString(), QgsMessageBar::WARNING, MBGuiApp::instance()->messageTimeout() );
  }
  ...
  if ( filterExpression.hasEvalError() )
  {
    MBGuiApp::instance()->messageBar()->pushMessage( tr( "Error filtering" ), filterExpression.evalErrorString(), QgsMessageBar::WARNING, MBGuiApp::instance()->messageTimeout() );
    return;
  }
