...
QgsStatusBarCoordinatesWidget::QgsStatusBarCoordinatesWidget( QWidget *parent )
    : QWidget( parent )
    , mDizzyTimer( nullptr )
    , mMapCanvases( nullptr ) //mMapCanvas( nullptr )
...
void QgsStatusBarCoordinatesWidget::setMapCanvases( QList<QgsMapCanvas *> *mapCanvases )
{
//    if ( mMapCanvases )
//    {
//      disconnect( mMapCanvas, SIGNAL( xyCoordinates( const QgsPoint & ) ), this, SLOT( showMouseCoordinates( const QgsPoint & ) ) );
...
//    }
    ...
    if ( mMapCanvases )
    {
        foreach(QgsMapCanvas *mapCanvas, *mMapCanvases)
        {
          if ( mapCanvas )
          {
            disconnect( mapCanvas, SIGNAL( xyCoordinates( const QgsPoint & ) ), this, SLOT( showMouseCoordinates( const QgsPoint & ) ) );
            disconnect( mapCanvas, SIGNAL( extentsChanged() ), this, SLOT( showExtent() ) );
          }
        }
    }

  mMapCanvases = mapCanvases;
  foreach(QgsMapCanvas *mapCanvas, *mapCanvases)
  {
    connect( mapCanvas, SIGNAL( xyCoordinates( const QgsPoint & ) ), this, SLOT( showMouseCoordinates( const QgsPoint & ) ) );
    connect( mapCanvas, SIGNAL( extentsChanged() ), this, SLOT( showExtent() ) );
  }
}
...
void QgsStatusBarCoordinatesWidget::validateCoordinates()
{
    ...
    foreach(QgsMapCanvas *mapCanvas, *mMapCanvases)
    {
      ...
      if ( mLineEdit->text() == "dizzy" )
      {
        ...
        {
          mDizzyTimer->stop();
          mapCanvas->setSceneRect( mapCanvas->viewport()->rect() );
          mapCanvas->setTransform( QTransform() );
        }
        else
        {
      ...
      {
        mapCanvas->setProperty( "retro", !mapCanvas->property( "retro" ).toBool() );
        refreshMapCanvas( mapCanvas );
        return;
      }
      ...
      mapCanvas->setCenter( QgsPoint( x, y ) );
      mapCanvas->refresh();
    }
}
...
void QgsStatusBarCoordinatesWidget::refreshMapCanvas(QgsMapCanvas * mapCanvas)
{
//    if ( !mMapCanvases )
//    {
//      return;
...
void QgsStatusBarCoordinatesWidget::showMouseCoordinates( const QgsPoint & p )
{
    ...
    foreach(QgsMapCanvas *mapCanvas, *mMapCanvases)
    {
      if ( !mapCanvas || mToggleExtentsViewButton->isChecked() )
      {
        return;
      }

      mLineEdit->setText( QgsCoordinateUtils::formatCoordinateForProject( p, mapCanvas->mapSettings().destinationCrs(),
