...
QMenu* QgsAppLayerTreeViewMenuProvider::createContextMenu()
{
  ...
  else if ( QgsLayerTreeNode* node = mView->layerTreeModel()->index2node( idx ) )
  {
    ...
    if ( QgsLayerTree::isGroup( node ) )
    {
      ...
      menu->addAction( QgsApplication::getThemeIcon( "/mActionRemoveLayer.svg" ), tr( "&Remove" ), MBGuiApp::instance(), SLOT( removeLayer() ) );

      menu->addAction( QgsApplication::getThemeIcon( "/mActionSetCRS.png" ),
                       tr( "&Set Group CRS" ), MBGuiApp::instance(), SLOT( legendGroupSetCRS() ) );

      menu->addAction( actions->actionRenameGroupOrLayer( menu ) );

      menu->addAction( tr( "&Set Group WMS data" ), MBGuiApp::instance(), SLOT( legendGroupSetWMSData() ) );
      ...
      if ( MBGuiApp::instance()->clipboard()->hasFormat( QGSCLIPBOARD_STYLE_MIME ) )
      {
        menu->addAction( tr( "Paste Style" ), MBGuiApp::instance(), SLOT( applyStyleToGroup() ) );
      }

      menu->addAction( tr( "Save As Layer Definition File..." ), MBGuiApp::instance(), SLOT( saveAsLayerDefinition() ) );
    ...
    else if ( QgsLayerTree::isLayer( node ) )
    {
      ...
      if ( rlayer )
      {
        menu->addAction( QgsApplication::getThemeIcon( "/mActionZoomActual.svg" ), tr( "&Zoom to Native Resolution (100%)" ), MBGuiApp::instance(), SLOT( legendLayerZoomNative() ) );

        if ( rlayer->rasterType() != QgsRasterLayer::Palette )
          menu->addAction( tr( "&Stretch Using Current Extent" ), MBGuiApp::instance(), SLOT( legendLayerStretchUsingCurrentExtent() ) );
      }

      menu->addAction( QgsApplication::getThemeIcon( "/mActionRemoveLayer.svg" ), tr( "&Remove" ), MBGuiApp::instance(), SLOT( removeLayer() ) );

      // duplicate layer
      QAction* duplicateLayersAction = menu->addAction( QgsApplication::getThemeIcon( "/mActionDuplicateLayer.svg" ), tr( "&Duplicate" ), MBGuiApp::instance(), SLOT( duplicateLayers() ) );

      if ( !vlayer || vlayer->geometryType() != QGis::NoGeometry )
      {
        // set layer scale visibility
        menu->addAction( tr( "&Set Layer Scale Visibility" ), MBGuiApp::instance(), SLOT( setLayerScaleVisibility() ) );

        // set layer crs
        menu->addAction( QgsApplication::getThemeIcon( "/mActionSetCRS.png" ), tr( "&Set Layer CRS" ), MBGuiApp::instance(), SLOT( setLayerCRS() ) );

        // assign layer crs to project
        menu->addAction( QgsApplication::getThemeIcon( "/mActionSetProjectCRS.png" ), tr( "Set &Project CRS from Layer" ), MBGuiApp::instance(), SLOT( setProjectCRSFromLayer() ) );
      }
      ...
      if ( layer && mView->selectedLayerNodes().count() == 1 )
      {
        ...
        MBGuiApp *app = MBGuiApp::instance();
      ...
      if ( vlayer )
      {
        QAction *toggleEditingAction = MBGuiApp::instance()->actionToggleEditing();
        QAction *saveLayerEditsAction = MBGuiApp::instance()->actionSaveActiveLayerEdits();
        QAction *allEditsAction = MBGuiApp::instance()->actionAllEdits();

        // attribute table
        menu->addAction( QgsApplication::getThemeIcon( "/mActionOpenTable.svg" ), tr( "&Open Attribute Table" ),
                         MBGuiApp::instance(), SLOT( attributeTable() ) );
        ...
        // save as vector file
        menu->addAction( tr( "Save As..." ), MBGuiApp::instance(), SLOT( saveAsFile() ) );
        menu->addAction( tr( "Save Temp..." ), MBGuiApp::instance(), SLOT( saveTempFile() ) );
        menu->addAction( tr( "Save As Layer Definition File..." ), MBGuiApp::instance(), SLOT( saveAsLayerDefinition() ) );

        if ( vlayer->dataProvider()->supportsSubsetString() )
        {
          QAction *action = menu->addAction( tr( "&Filter..." ), MBGuiApp::instance(), SLOT( layerSubsetString() ) );
      ...
      {
        menu->addAction( tr( "Save As..." ), MBGuiApp::instance(), SLOT( saveAsRasterFile() ) );
        menu->addAction( tr( "Save As Layer Definition File..." ), MBGuiApp::instance(), SLOT( saveAsLayerDefinition() ) );
      }
      ...
      if ( layer && QgsProject::instance()->layerIsEmbedded( layer->id() ).isEmpty() )
        menu->addAction( tr( "&Properties" ), MBGuiApp::instance(), SLOT( layerProperties() ) );
    ...
    for ( int i = 0; i < lyrActions.count(); i++ )
    {
      if ( lyrActions[i].allLayers || lyrActions[i].layers.contains( layer ) )
      {
        ...
        else
        {
          ...
          // adapted from MBGuiApp::getPluginMenu( QString menuName )
