...
bool QgsGuiVectorLayerTools::startEditing( QgsVectorLayer* layer ) const
{
  ...
  if ( !layer->isEditable() && !layer->isReadOnly() )
  {
    ...
    {
      MBGuiApp::instance()->messageBar()->pushMessage( tr( "Start editing failed" ),
          tr( "Provider cannot be opened for editing" ),
          QgsMessageBar::INFO, MBGuiApp::instance()->messageTimeout() );
      return false;
    }
  ...
  if ( layer->isModified() )
  {
                                       ...
                                       buttons ) )
    {
      ...
      case QMessageBox::Discard:
        MBGuiApp::instance()->mapCanvas()->freeze( true );
        if ( !layer->rollBack() )
        {
          MBGuiApp::instance()->messageBar()->pushMessage( tr( "Error" ),
              tr( "Problems during roll back" ),
              QgsMessageBar::CRITICAL );
          res = false;
        }
        MBGuiApp::instance()->mapCanvas()->freeze( false );
...
void QgsGuiVectorLayerTools::commitError( QgsVectorLayer* vlayer ) const
{
  ...
  QgsMessageBarItem *errorMsg = new QgsMessageBarItem(
    ...
    0,
    MBGuiApp::instance()->messageBar() );
  MBGuiApp::instance()->messageBar()->pushItem( errorMsg );
