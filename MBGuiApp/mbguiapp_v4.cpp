...
//    canvas_xy = new QgsMapCanvas( centralWidget, "theMapCanvas" );
...
//    canvas_xy->setWhatsThis( tr( "Map canvas. This is where raster and vector "
...
//    mCentralContainer->insertWidget( 0, canvas_xy );
////    mCentralContainer->insertWidget( 1, mWelcomePage );

//    centralLayout->addWidget( mCentralContainer, 0, 0, 2, 1 );

//    connect( canvas_xy, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
...
//    mLayerTreeView->show();
    ...
    provider = new MyMenuProvider(mLayerTreeView,undoStack);
    mLayerTreeView->setMenuProvider(provider);
...
//    connect( canvas_xy, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
    ...
    canvas_yz->setWhatsThis( tr( "YZ-Map canvas. This is where the YZ-plots of vector "
                                  "layers are displayed when added to the map" ) );
//    connect( canvas_yz, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
    ...
    canvas_error->setWhatsThis( tr( "Error-Map canvas. This is where the Error-plots of vector "
                                  "layers are displayed when added to the map" ) );
//    connect( canvas_error, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
    ...
    connect( canvas_xy, SIGNAL( messageEmitted( const QString&, const QString&, QgsMessageBar::MessageLevel ) ),
             this, SLOT( displayMessage( const QString&, const QString&, QgsMessageBar::MessageLevel ) ) );
    canvas_rms->setWhatsThis( tr( "RMS-Map canvas. This is where the RMS-plots of vector "
                                  "layers are displayed when added to the map" ) );
//    connect( canvas_rms, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
    ...
    canvas_mean->setWhatsThis( tr( "Mean-Map canvas. This is where the Mean-plots of vector "
                                  "layers are displayed when added to the map" ) );
//    connect( canvas_mean, SIGNAL( layersChanged() ), this, SLOT( showMapCanvas() ) );
...
//    connect( mActionChangeLabelProperties, SIGNAL( triggered() ), this, SLOT( changeLabelProperties() ) );
    ...
    #endif
    */
    qDebug() <<"Testing 6..." << endl;
}
void MBGuiApp::createActionGroups()
{
  qDebug() <<"Testing 7..." << endl;
  // Map Tool Group
...
void MBGuiApp::createMenus()
{
    ...
    viewMenu2 = menuBar()->addMenu(tr("View"));
    viewMenu2->addAction(tab1ToggleAction);
    viewMenu2->addAction(tab2ToggleAction);
    connect(viewMenu2, SIGNAL(aboutToShow()),this, SLOT(viewMenuAboutToShow1()));
    connect(viewMenu2, SIGNAL(aboutToHide()),this, SLOT(viewMenuAboutToHide1()));
    connect(viewMenu2, SIGNAL(aboutToShow()),this, SLOT(viewMenuAboutToShow2()));
    connect(viewMenu2, SIGNAL(aboutToHide()),this, SLOT(viewMenuAboutToHide2()));
...
//  mHelpMenu->insertAction( before, actionWhatsThis );
  qDebug() <<"Testing 10..." << endl;
}

void MBGuiApp::createToolBars()
{
qDebug() <<"Testing 11..." << endl;
  QSettings settings;
  ...
  toolbarMenuToolBars << mFileToolBar  << mLayerToolBar
//  << mDigitizeToolBar
...
qDebug() <<"Testing 12..." << endl;
}

void MBGuiApp::createStatusBar2()
{
    qDebug() <<"Testing 13..." << endl;
...
//      viewMenu->addAction(tab2ToggleAction);
      ...
      viewMenu2->addAction(dock1->toggleViewAction());
      viewMenu2->addAction(dock2->toggleViewAction());
...
void MBGuiApp::itemClicked(QtCanvasItem *item)
{
    updateExpandState();
...
void MBGuiApp::valueChanged(QtProperty *property, const QColor &value)
{
    if (!propertyToId.contains(property))
        return;
...
void MBGuiApp::valueChanged(QtProperty *property, const QPoint &value)
{
    if (!propertyToId.contains(property))
        return;
...
void MBGuiApp::valueChanged(QtProperty *property, const QSize &value)
{
    if (!propertyToId.contains(property))
        return;
...
{
    qDebug()<<"SIZE"<<endl;
    qDebug()<<mLayerTreeView->size()<<endl;
    if (mLayerTreeView->selectedLayerNodes().isEmpty())
        return;

    QUndoCommand *deleteCommand = new DeleteCommand(mLayerTreeView);
    undoStack->push(deleteCommand);
}
void MBGuiApp::addLayer()
{
    QUndoCommand *addCommand = new AddCommand(root,canvas_xy,canvas_yz,myLayerSet,mLayerTreeView);
    undoStack->push(addCommand);
}
void MBGuiApp::addGroup()
{
    QUndoCommand *addGroupCommand = new AddGroupCommand(mLayerTreeView);
...
void MBGuiApp::viewMenuAboutToHide1()
{
    tab1ToggleAction->setEnabled(true);
}
void MBGuiApp::viewMenuAboutToShow1()
{
//    tab1ToggleAction->setEnabled(!tab->isTabEnabled(tab->indexOf(propertyEditor)));
}
void MBGuiApp::viewMenuAboutToHide2()
{
    tab2ToggleAction->setEnabled(true);
}
void MBGuiApp::viewMenuAboutToShow2()
{
...
{
    statusBar()->showMessage(tr("Ready Sams"));
}
...
//  }
  ...
  qDebug() <<"Testing 16..." << endl;
}

void MBGuiApp::setupConnections()
{
    qDebug() <<"Testing 17..." << endl;
...
qDebug() <<"Testing 18..." << endl;
}

void MBGuiApp::createCanvasTools()
{
    qDebug() <<"Testing 19..." << endl;
  // create tools
...
qDebug() <<"Testing 20..." << endl;
}

void MBGuiApp::createOverview()
{
    qDebug() <<"Testing 21..." << endl;
  // overview canvas
...
//  mLayerTreeCanvasBridge->setAutoEnableCrsTransform( otfTransformAutoEnable );
  ...
  qDebug() <<"Testing 34..." << endl;
}

void MBGuiApp::updateNewLayerInsertionPoint()
{
    qDebug() <<"Testing 35..." << endl;
  // defaults
...
/*void MBGuiApp::addLayerDefinition()
{
...
{
  canvas_xy->setMapTool( mMapTools.mSelectFeatures );
}

void MBGuiApp::selectByPolygon()
{
  canvas_xy->setMapTool( mMapTools.mSelectPolygon );
}

void MBGuiApp::selectByFreehand()
{
  canvas_xy->setMapTool( mMapTools.mSelectFreehand );
}

void MBGuiApp::selectByRadius()
{
  canvas_xy->setMapTool( mMapTools.mSelectRadius );
}
...
QgsMapLayer *MBGuiApp::activeLayer()
{
  return mLayerTreeView ? mLayerTreeView->currentLayer() : nullptr;
}

/** Set the current layer */
bool MBGuiApp::setActiveLayer( QgsMapLayer *layer )
{
  if ( !layer )
    return false;
