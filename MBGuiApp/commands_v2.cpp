...
// ADD COMMAND
//******************************************************************
AddCommand::AddCommand(QgsLayerTreeGroup * root, QgsMapCanvas * canvas_xy, QgsMapCanvas * canvas_yz,QList<QgsMapCanvasLayer> myLayerSet,QList<QgsMapCanvasLayer> myLayerSet_yz, QgsLayerTreeView * view, QMap<QString,QStack<QgsFeatureIds>*> *selectedMap, QMap<QString,QString> *layerSuffixes,QUndoCommand *parent) : QUndoCommand(parent)
{
...
//                          query = """ SELECT rowid AS gid,ST_PointOnSurface(geometry)AS geom FROM '""" + mypLayer.name() + """' """;
//                          query = """ SELECT * FROM '""" + mypLayer.name() + """' """;
//                          QString vLayerName = mypLayer.name();
//                          QgsVectorLayer vlayer = QgsVectorLayer( "?query={}".format(query), vLayerName.append("_yz"), "virtual" );
//                          QgsMapLayerRegistry.instance().addMapLayer(vlayer);


//                QgsVectorLayer *vLayer    = new QgsVectorLayer(uri, myRasterFileInfo.completeBaseName(), "virtual");
                ...
                QString vectorFilename = QDir::temp().path().append("/MBGuiApp").append("/Temp_").append(mypLayer->name()).append(".shp");
                ...
                error = QgsVectorFileWriter::writeAsVectorFormat(
                          ...
                          true,
                          &errorMessage,
                          datasourceOptions, //ogr data source creation options
                            layerOptions,//dialog->layerOptions(), //ogr layer creation options
    ...
    //            uri = "file:///" + myFileName + "?&crs=epsg:4723&wktField=shape";
    ////            uri = "file:///" + myFileName + "?&useHeader=no&type=whitespace&xField=1&yField=2&zField=3crs=epsg:4326&wktField=shape&index=yes";
    //            mypLayer->setDataSource(uri, myRasterFileInfo.completeBaseName(), "ogr");
...
//        }
       ...
       ok = QgsMapLayerRegistry::instance()->addMapLayer(mypLayer, TRUE);

//       if ( !ok )
//       {
//         // we have no idea what this file is...
//     //    QgsMessageLog::logMessage( tr( "Unable to load %1" ).arg( myRasterFileInfo.completeBaseName() ) );
//         QMessageBox::critical(parent, tr("MBGui: Open File Error"),tr( "Unable to load %1" ).arg( myFileName ));
//       }
       ...
       // Add the Layer to the Layer Set
        ...
        QString query = """?query=SELECT *, field_1 AS field_2, field_2 AS field_1 FROM '""" + mypLayer->name() + """' """;
//        QString query = "?query=SELECT * FROM road WHERE type = 'Expressway'";
//        QString query = """ SELECT * FROM '""" + mypLayer->name() + """' """;
        QString vLayerName = mypLayer->name();
        QgsVectorLayer *vlayer = new QgsVectorLayer( query, vLayerName.append("_yz"), "virtual" );
//        QgsVectorLayer vlayer = QgsVectorLayer( "?query={}".format(query), vLayerName.append("_yz"), "virtual" );
        QgsMapLayerRegistry::instance()->addMapLayer(vlayer,false);
        myLayerSet_yz.append(QgsMapCanvasLayer(vlayer));
        canvas_yz->setExtent(vlayer->extent());
